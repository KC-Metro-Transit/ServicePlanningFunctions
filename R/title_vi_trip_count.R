...
#'
#'
count_trips_by_geography_title_vi <- function(
  gtfs_object,
  project_name,
  geography = NULL,
  netplan_gtfs,
  ...
  if (is.null(geography)) {
    ...
    !(("MULTIPOLYGON" %in% class(geography)) | "POLYGON" %in% class(geography))
  ) {
    cli::cli_abort(c(
      "Object Type Error",
  ...
  calendar <- gtfs_calendar_full_dates(
  ...
  if (netplan_gtfs == TRUE) {
    ...
    if (include_st == FALSE) {
      trips_by_geo_rte <- trips_by_geo_rte %>%
        ...
        ) %>% #for title VI filter out all non metro services
        ...
        dplyr::mutate(
          arrival_time = stringr::str_replace(
            arrival_time,
        ...
        dplyr::filter(
          seconds_after_midnight == first_seconds_after_midnight
        ) %>%
      ...
      #allowing for situations with ST routes should be included in analysis. (doesn't filter out 500 series)
      ...
      trips_by_geo_rte <- trips_by_geo_rte %>%
        ...
        ) %>% #filter out everything but metro and ST
        #this step is fixing the times after midnight issue. I cheated
        # and assigned all trips after midnight to 4 am becuase it doesn't
        #really matter when the trips arrive in the early am block for
        #this analysis 2021.10.05
    ...
    #handling the difference between netplan GTFS which uses route_id for storage of route num and production GTFS which uses route_short_name
    trips_by_geo_rte <- stops_geo %>%
      dplyr::select(stop_id, GEOID) %>%
      dplyr::left_join(
        gtfs$stop_times,
    ...
    if (include_st == FALSE) {
      ...
      trips_by_geo_rte <- trips_by_geo_rte %>%
        ...
        ) %>% #for title VI filter out everything but Metro and ST
        #this step is fixing the times after midnight issue. I cheated
        # and assigned all trips after midnight to 4 am becuase it doesn't
        #really matter when the trips arrive in the early am block for
        #this analysis 2021.10.05
