...
#'
#'
count_trips_by_geography <- function(
  gtfs_object,
  begin_time,
  end_time,
  analysis_period,
  ...
  start_time_sec <- (as.numeric(start_time_vec[, 1]) * 3600) +
    as.numeric(start_time_vec[, 2]) * 60 +
    as.numeric(start_time_vec[, 3])

  end_time_vec <- stringr::str_split_fixed(end_time, pattern = ":", 3)

  end_time_sec <- (as.numeric(end_time_vec[, 1]) * 3600) +
    as.numeric(end_time_vec[, 2]) * 60 +
    as.numeric(end_time_vec[, 3])

  #get geography #####

  if (geography == "block_group") {
    acs <- sf::read_sf(fs::path_package(
      "extdata",
      "blkgrp20_shore.shp",
  ...
  if (
    !(("tidygtfs" %in% class(gtfs_object)) |
      ("gtfs" %in% class(gtfs_object)) |
      ("list" %in% class(gtfs_object)))
  ) {
    cli::cli_abort(c(
      "Object Type Error",
  ...
  routes <- clean_service_rte_num(gtfs$routes, netplan_gtfs = netplan_gtfs) %>%
    ...
    dplyr::mutate(
      vehicle_capacity = dplyr::case_when(
        (service_rte_num < 400 &
          !(service_rte_num %in% c(200, 204, 224, 96, 98))) ~ 61,
  ...
  stop_times <- stringr::str_split_fixed(
    gtfs$stop_times$arrival_time,
    pattern = ":",
    3
  )

  stop_time_sec <- (as.numeric(stop_times[, 1]) * 3600) +
    as.numeric(stop_times[, 2]) * 60 +
    as.numeric(stop_times[, 3])
  ...
  calendar_full_dates <- gtfs_calendar_full_dates(
    ...
    dplyr::summarize(
      weekly_trip_ct = sum(ct) / max(num_of_weeks),
      .groups = "keep"
    )
  ...
  calendar <- calendar_full_dates %>%
    dplyr::filter(
      day_of_week %in%
        unlist(keep(day_type_dim, names(day_type_dim) == day_type))
    ) %>%
  ...
  trips_by_geo_rte <- stops_geo %>%
    ...
    dplyr::filter(
      !(service_rte_num %in%
        c(97, 90, 560:595, 629, 632:634, 636:662, 680:772, 776:900, 932:999))
    ) %>%
    dplyr::filter(service_id %in% calendar$service_id) %>%
    dplyr::filter(
      seconds_after_midnight >= start_time_sec &
        seconds_after_midnight <= end_time_sec
    ) %>%
    ...
    dplyr::mutate(
      first_seconds_after_midnight = min(seconds_after_midnight)
    ) %>%
