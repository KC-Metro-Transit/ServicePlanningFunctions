...
get_title_vi_classification <- function(
  acs_year,
  geography = NULL,
  state = "WA",
  county = "King"
  ...
  tract_classification <- DBI::dbGetQuery(
    con,
    paste0(
      "SELECT [ACS_YR]
            ,[GEOID]
  ...
  census_table <- tidycensus::get_acs(
    geography = "tract",
    variables = c(
    ...
    dplyr::mutate(
      population_200_pct_below_poverty_line = pov1 - pov2,
  ...
  tract_info <- acs %>%
    ...
    dplyr::mutate(
      low_income_tract_flag = dplyr::case_when(
        is.na(low_income_tract_flag) ~ 0,
        TRUE ~ low_income_tract_flag
      )
    ) %>%
    dplyr::mutate(
      bipoc_tract_flag = dplyr::case_when(
        is.na(bipoc_tract_flag) ~ 0,
        TRUE ~ bipoc_tract_flag
      )
    )
