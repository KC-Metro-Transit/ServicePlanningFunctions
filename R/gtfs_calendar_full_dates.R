...
gtfs_calendar_full_dates <- function(
  calendar,
  calendar_dates = NULL,
  netplan_gtfs,
  designated_start_date = NULL,
  designated_end_date = NULL
) {
  server <- "kcitazrsqlprp01.database.windows.net"
  database = "tbird_dw"

  can_connect_tbird <- DBI::dbCanConnect(
    odbc::odbc(),
  ...
  if (netplan_gtfs == FALSE) {
    if (missing(calendar_dates)) {
      cli::cli_abort(c(
        "X" = "calendar_dates.txt GTFS file must be defined in the calendar_dates argument."
      ))
    } else if (anyNA(cal_df$start_date) | anyNA(cal_df$end_date)) {
      cli::cli_abort(c(
        "X" = 'Calendar GTFS is missing start dates or end dates. Set netplan_gtfs to "TRUE" if GTFS file is from Netplan.'
      ))
    } else {
      ...
      cli::cli_alert_info(
        "Generating dates from {designated_start_date} to {designated_end_date}."
      )
    }
  } else if (netplan_gtfs == TRUE) {
    if (!anyNA(cal_df$start_date) & !anyNA(cal_df$end_date)) {
      #combine gtfs function has a way to add cal dates to NetPlan GTFS now. Changing this to use existing to allow that to work.
      cal_dates_df <- calendar_dates
      cli::cli_alert_info("Start Date and End Date found in Netplan GTFS file.")
      if (!missing(designated_start_date) & !missing(designated_end_date)) {
        cli::cli_alert_warning(
          "Ignoring Designated Start Date and Designated End Date values."
        )
      }
      designated_start_date <- min(as.Date(cal_df$start_date))
      designated_end_date <- max(as.Date(cal_df$end_date))
      ...
      cli::cli_abort(c(
        "X" = "Start Date and End Date must be defined when using GTFS files generated from Netplan (YYYYMMDD)."
      ))
    } else if (
      stringr::str_detect(designated_start_date, "^\\d{4}-\\d{2}-\\d{2}$") &&
        stringr::str_detect(designated_end_date, "^\\d{4}-\\d{2}-\\d{2}$")
    ) {
      cli::cli_alert_info(
        "Generating dates from {designated_start_date} to {designated_end_date}."
      )
      ...
      is.numeric(designated_start_date) && is.numeric(designated_end_date)
    ) {
      cli::cli_alert_info(
        "Generating dates from {designated_start_date} to {designated_end_date}."
      )
      ...
      cli::cli_abort(c(
        "X" = " Designated Start Date and Designated End Date must be set as YYYYMMDD."
      ))
    }
  }

  # Pull all dates from EDW.DIM_DATE from T-Bird between start date and latest end date
  dim_date <- DBI::dbGetQuery(
    con,
    paste0(
  ...
  cal_comb <- cal_full_dates_df %>%
    dplyr::left_join(
      cal_dates_df,
      by = c("service_id", "full_date" = "date", "name")
    ) %>%
    ...
    dplyr::mutate(
      ...
      ct = dplyr::coalesce(
        ifelse(exception_type == 2, 0, exception_type),
        value
      )
    ) %>%
    dplyr::select(!c(name)) %>%
