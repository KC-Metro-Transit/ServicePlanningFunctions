...
combine_gtfs <- function(
  ...
  if (!dir.exists(gtfs_filepath)) {
    ...
    if (length(stringr::str_subset(gtfs_folders, pattern = "txt")) != 0) {
      ...
      for (i in 1:length(gtfs_folders)) {
        weekday_gtfs <- list.files(
          path = here::here(gtfs_filepath, gtfs_folders[i]),
          pattern = "*.txt"
        )
        weekday_gtfs <- weekday_gtfs[
          !weekday_gtfs %in%
            c(
              "stops.txt",
              "stop_times.txt",
        ...
        weekday <- lapply(weekday_gtfs, function(x) {
          out <- readr::read_csv(
            here::here(gtfs_filepath, gtfs_folders[i], x),
            col_names = T,
            show_col_types = F,
            progress = F
          )
  ...
  calendar <- baseline_gtfs[stringr::str_detect(
    ...
    dplyr::distinct() %>%
    dplyr::mutate(
      start_date = lubridate::ymd(designated_start_date), #add calendar dates
  ...
  calendar_csv <- baseline_gtfs[stringr::str_detect(
    ...
    dplyr::distinct() %>%
    dplyr::mutate(
      start_date = designated_start_date, #add calendar dates
  ...
  repeat {
    ...
    shapes_temp <- shapes %>%
      dplyr::mutate(
        ...
        flag = ifelse(
          shape_id == prev_id &
            (shape_pt_lat != prev_lat | shape_pt_lon != prev_lon) &
            shape_dist_traveled == prev_dist,
          1,
          0
        )
      ) %>%
